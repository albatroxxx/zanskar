...
| 3.5 Protocol expansion | in progress | SSH file transfer over SFTP (ADR 0016); managed/PaaS database access via an ephemeral per-session client container (ADR 0017) | in progress |
...
**deferred**: a live-QA pass turned up rough edges worth fixing, and we chose to
broaden and harden the feature surface — Phase 3.5, the protocol-expansion work
(ADR 0016, ADR 0017) — before stamping 1.0. The release pipeline stays ready; the
tag is not cut until that work lands and testing signs off.
...
- [ ] Tag v1.0.0 — **deferred** until Phase 3.5 lands and testing signs off.
