...
| Category | Threat | Mitigation |
|---|---|---|
| Spoofing | Attacker redirects the gateway to a hostile host that harvests credentials | SSH host key pinning: trust on first use during enrollment, admin must approve the fingerprint before users can connect, any later change blocks connections until re-approved. The enrollment probe deliberately skips verification (there is nothing to verify against yet), so an attacker on the path at probe time could have their key or certificate pinned; the admin is expected to compare the shown fingerprint with the host out of band before approving. RDP: TLS certificate pinning with the same approval flow. VNC and WinRM: TLS required where the protocol supports it |
